...
BME4 uses a specially designed PBR material system. Each material consists of three 2048×2048 `.png` texture images.
...
![](@site/static/img/texture/texture-albedo.jpg)
...
![](@site/static/img/texture/texture-normal.jpg)

:::tip

In [Adobe Photoshop](https://www.adobe.com/products/photoshop.html), you can use `Filter → 3D → Generate Normal Map...` to generate a normal map.
...
![](@site/static/img/texture/texture-mask-r.jpg)

- `G` channel: smoothness. (Optional, black if omitted)

![](@site/static/img/texture/texture-mask-g.jpg)

- `B` channel: metallic. (Optional, black if omitted)

![](@site/static/img/texture/texture-mask-b.jpg)

- `A` channel: ambient occlusion. (Optional, white if omitted)

![](@site/static/img/texture/texture-mask-a.jpg)

## Import Texture

In `Assets → Textures` panel, click the plus sign at the bottom, and choose images to import.

Renaming textures in BME4 is not supported now, but you can rename them by changing their file names in `%USERPROFILE%\AppData\LocalLow\Mushreb\BME4\Assets\Textures`.

## Create Material

In `Assets → Materials` panel, click the plus sign at the bottom to create a material, or double-click one to edit it.

When editing a material, click `Save` to save it, click `Reload` or close the window to discard any changes.

Renaming materials in BME4 is not supported now, but you can rename them by changing their file names in `%USERPROFILE%\AppData\LocalLow\Mushreb\BME4\Assets\Materials`.
